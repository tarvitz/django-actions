...
{% block content %}
	{% if files %}
		{% if perms.files.delete_file or perms.files.change_file %}
			<form action='.' method='post'>
			{% comment %} you may also use {{ form.as_ul }}
			but first you should alter apps.actions.forms and
			set items widget to MultipleHiddenInput()
			{% endcomment %}
			{{ form.action.errors }}
			{{ form.action }}
		...
		{% for f in files %}
			<li>
			{% if perms.files.delete_file or perms.files.change_file %}
				<input type='checkbox' name='items' value='{{ f.id }}'>
			{% endif %}
